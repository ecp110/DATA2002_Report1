...
cv_full = train(
  data.quality ~ data.alcohol + data.volatile.acidity + data.total.sulfur.dioxide, dataNew,
  method = "lm",
...
results <- resamples(list(simple = simple_cv, step = step_cv, full = full_cv))
summary(results)
...
modelIdeal = lm(data.quality ~ data.alcohol + data.volatile.acidity + data.total.sulfur.dioxide, data = dataNew)
...
modelIdeal = lm(quality ~ ., data = data)
autoplot(modelIdeal, which=1:2)
```
...
modelIdeal = lm(quality ~ alcohol, data = data)
autoplot(modelIdeal, which=1:2)
```